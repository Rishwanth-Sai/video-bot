import instaloader
import os
from utils import count_downloaded_videos
    ...
    try:
        ...
        for post in posts:
            if count_downloaded_videos() >= TOTAL_DOWNLOAD_LIMIT:
                print("\nDownload limit reached. Stopping all downloads.")
                break

            if video_count >= limit:
                print(f"\nReached {limit} videos for profile {profile}. Moving to next profile.")
                break
            ...
            if post.is_video:
                ...
                if not os.path.exists(video_filename):
                    print(f"\nDownloading Instagram video: {post.shortcode}")
                    ...
                    print(f"\nVideo {post.shortcode} already exists. Skipping.")
